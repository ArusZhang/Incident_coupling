...
def GetInitialHeatFlux():
    ...
    for i in range(0,2*N_furnace_points):
        ...
        for j in range(0,N_reactor):
            ...
            HeatFluxTemp[j][i] = Heatflux_Furnace[j][i]/1000.0
        ...
        for i in range(0,N_reactor):
            ...
            for j in range(0,N_reactor_axial):
                if j==0:
                    ...
                    if FlowRate[i]>100:
                        flowrate_content[i] += str('%1.3f' % float(2*FlowRate[i]))+' '
                    elif FlowRate[i]>10:
                        flowrate_content[i] += str('%1.4f' % float(2*FlowRate[i]))+' '
                    else:
                        flowrate_content[i] += str('%1.5f' % float(2*FlowRate[i]))+' '
...
def WriteCokeFile():
    ...
    for i in range(0,N_reactor):
        ...
        FileCoke = open(WorkDir+'\\Projects\\SEI\\coke.i','w')
        CokePre = '500    0.1\n'+str(Interpo)+'\n'
...
def SimulateReactors():
    ...
    while ((iteration<MaxCIPIteration) and (FlagConv==0)):
        ...
        for i in range(0,N_reactor):
            FileCSV = open(WorkDir+'\\Projects\\SEI\\general_info.csv','rb')
            ...
            if FlagConvArray[i]==0:
                ...
                if COP[i]<1.125:
                    ...
                    if CounterCIP[i]==1:
                        ...
                        x_old[i]=CIP[i]
                        # CIP cannot exceed the range of the two limits
                        if (x_low[i]!=0 and x_high[i]!=0):
                            if x_new[i]<x_low[i] or x_new[i]>x_high[i]:
...
def WriteResults():
    ...
    for i in range(0,N_reactor):
        ...
        FileCSV_info = open(WorkDir+'\\Projects\\SEI\\general_info.csv','rb')
        ...
        FileTXT = open(WorkDir+'\\Projects\\SEI\\results_summary.txt','r')
        for j,line in enumerate(FileTXT):
            if j==13:
                HeatFluxTotal[i]=float(line.split()[4])
        FileTXT.close()
        # read yields.csv
        FileCSV_yield = open(WorkDir+'\\Projects\\SEI\\yields.csv','rb')
    ...
    if CoupledSim==1:
        ...
        wb.save(WorkDir+'\\Projects\\SEI\\Reactor_Results_'+CaseName+'_timestep'+str(TimeStep)+'_PEloop'+str(IterationPE)+'_it'+str(IterationTMT)+'.xls')
    else:
        sheet8.write(8,0,'Heat flux scaling ratio')
        sheet8.write(8,4,float(HeatFluxScalingRatio))
        wb.save(WorkDir+'\\Projects\\SEI\\Reactor_Results_'+CaseName+'_timestep'+str(TimeStep)+'_PEloop'+str(IterationPE)+'.xls')
    ...
    shutil.copytree(WorkDir+'\\Projects\\SEI',Path)
    # remove the summary excel file
    if CoupledSim==1:
        os.remove(WorkDir+'\\Projects\\SEI\\Reactor_Results_'+CaseName+'_timestep'+str(TimeStep)+'_PEloop'+str(IterationPE)+'_it'+str(IterationTMT)+'.xls')
        if IterationTMT>1:
            os.remove(WorkDir+'\\Projects\\SEI\\SEI_heatflux_'+CaseName+'_timestep'+str(TimeStep)+'_PEloop'+str(IterationPE)+'_it'+str(IterationTMT)+'.xls')
    else:
        os.remove(WorkDir+'\\Projects\\SEI\\Reactor_Results_'+CaseName+'_timestep'+str(TimeStep)+'_PEloop'+str(IterationPE)+'.xls')
...
def GenerateWalltempCoefficient():
    ...
    for i in range(0,N_reactor_axial):
        if Coilsim_Axial[i] <= 12.36:
            Furnace_Position[i] = 14.27 - Coilsim_Axial[i]
        elif Coilsim_Axial[i] <= AxialDivPointLeg:
            Furnace_Position[i] = 1.91 - 0.76*(math.sin(Coilsim_Axial[i]-12.36))
        elif Coilsim_Axial[i] <= AxialDivPointLeg+PI/2.0:
            Furnace_Position[i] = 0.76 - 0.76*math.cos(Coilsim_Axial[i]-AxialDivPointLeg) + 1.15
        else:
            Furnace_Position[i] = Coilsim_Axial[i] - (AxialDivPointLeg+PI/2.0) + 1.91
    # TMT values in Kelvin
    for i in range(0,N_reactor):
        TMTinKelvin=array(TMT[i])+273.15
        # regression
        TMTCoefInlet[i] = polyfit(Furnace_Position[1:mark],TMTinKelvin[1:mark],6)
        TMTCoefOutlet[i] = polyfit(Furnace_Position[mark:-6],TMTinKelvin[mark:-6],6)
...
def FurnaceEstimation():
    ...
    for i in range(0,N_reactor):
        for j in range(0,2*N_furnace_points):
            if j==0:
                # the first segment
                HeatAbsorbedSingleCoil[i] += Heatflux_Furnace[i][j]*Furnace_Axial[j]*PI*InletID*2.0
            else:
                # inlet legs
                if Furnace_Axial[j-1]<AxialDivPointDiameter:
                    HeatAbsorbedSingleCoil[i] += Heatflux_Furnace[i][j-1]*(Furnace_Axial[j]-Furnace_Axial[j-1])*PI*InletID*2.0
                # outlet legs
                else:
        ...
        Q_absorb += HeatAbsorbedSingleCoil[i]*12.0
    ...
    while LoopEnd==False:
        ...
        E_N2=(E_N2*8.3145-0.0)*0.7203
        E_O2=(E_O2*8.3145-0.0)*0.0174
        E_CO2=(E_CO2*8.3145+393510.0)*0.0858
        E_H2O=(E_H2O*8.3145+241826.0)*0.1765
...
def GenerateHeatflux():
    ...
    for i in range(0,N_reactor):
        ...
        for j in range(0,N_furnace_points):
            if Furnace_Axial[j]<11.27:
                Heatflux_Furnace[i][j] = heatflux_inlet[i][j]*63.0/51.0
            else:
                Heatflux_Furnace[i][j] = heatflux_inlet[i][j]*87.0/73.0
            Heatflux_Furnace[i][N_furnace_points+j] = heatflux_outlet[i][j]*87.0/73.0
    ...
    writebook.save(WorkDir+'\\Projects\\SEI\\SEI_heatflux_'+CaseName+'_timestep'+str(TimeStep)+'_PEloop'+str(IterationPE)+'_it'+str(IterationTMT+1)+'.xls')
...
def GetHeatFlux():
    ...
    for i in range(0,N_reactor):
        for j in range(0,2*N_furnace_points):
            # convert heat flux to (kcal/m2)
            HeatFluxTemp[j] = Heatflux_Furnace[i][j]/1000.0
    # interpolate heatflux to COILSIM axial positions
...
InletID=0.051                   # inner diameter of inlet leg (m)
InletOD=0.063                   # inner diameter of outlet leg (m)
AxialDivPointLeg=12.36+PI/2.0   # dividing point of the inlet and outlet legs (m)
AxialDivPointDiameter=11.27     # dividing point of the inlet and outlet diameters (m)
AxialEndPoint=12.36*2.0+PI      # ending point of the reactor coil (m)
LossRatio=0.03                  # heat loss ratio through furance refractory
...
if os.path.exists(WorkDir+'\\Projects\\SEI'): shutil.rmtree(WorkDir+'\\Projects\\SEI')
time.sleep(4.0)
shutil.copytree(TempDir,WorkDir+'\\Projects\SEI')
# remove the summary excel file
os.remove(WorkDir+'\\Projects\\SEI\\'+FileReactor)
    ...
    while TimeStepLoopFin==False:
        ...
        while PELoopConv==False:
            ...
            while TMTLoopConv==False:
                ...
                if IterationTMT==1:
                    PEforTMTloop=MixingCupPE
                if IterationTMT>1:
                    PEforTMTloop_old=PEforTMTloop
                    PEforTMTloop=MixingCupPE
                    PEforTMTloopError=abs(PEforTMTloop-PEforTMTloop_old)
                    # check the convergence of the TMT loop
                    ErrorTMT=abs(array(TMT)-array(TMT_old))
                    ErrorTMT=[x[1:] for x in ErrorTMT]
                    TMTcounter=0
                    for i in range(0,N_reactor):
                        for j in range(0,N_reactor_axial-1):
                            if ErrorTMT[i][j]>1:
                                TMTcounter+=1
                    MaxErrorTMT=np.max(ErrorTMT)
                    AvgErrorTMT=np.average(ErrorTMT)
                    print '        Maximum TMT error: '+ str(MaxErrorTMT)
                    if MaxErrorTMT<TMTTreshold and AvgErrorTMT<4:
                        TMTLoopConv=True
                    if TMTcounter<8 and AvgErrorTMT<4:
                        TMTLoopConv=True
            ...
            if PELoopConv==False:
                ...
                if IterationPE==1:
                    ...
                    if PE_now>MixingCupPEtarget:
                        ...
                        Fuel_high=FuelScalingRatio
                    # calculate new fuel flow rate scaling ratio
                    if PE_old==PE_now:
                        FuelScalingRatio_new=FuelScalingRatio+(FuelScalingRatio_old-FuelScalingRatio)/0.0001*(MixingCupPEtarget-PE_now)
                    else:
